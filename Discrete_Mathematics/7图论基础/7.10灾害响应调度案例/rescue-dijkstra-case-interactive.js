(function () {
  ...
  function draw() {
    ...
    nodes.forEach(item => {
      ...
      ctx.save();
      ctx.shadowColor = isTarget ? "rgba(214, 59, 29,0.28)" : "rgba(116,55,31,0.14)";
      ctx.shadowBlur = isTarget || isCurrent || isHover ? 18 : 8;
      ctx.beginPath();
      ctx.arc(p.x, p.y, r, 0, Math.PI * 2);
      ctx.fillStyle = isCenter ? "#d63b1d" : isCurrent ? "#f97316" : isVisited ? "#2f7d57" : "#fff7ed";
      ctx.fill();
      ctx.shadowBlur = 0;
      ctx.lineWidth = isTarget ? 4 : 2;
      ctx.strokeStyle = isTarget ? "#d63b1d" : isVisited ? "#2f7d57" : "rgba(116,55,31,0.24)";
      ctx.stroke();
      ctx.fillStyle = isCenter || isCurrent || isVisited ? "#fff" : "#7c2d12";
      ctx.font = "900 15px 'Noto Serif SC', 'Microsoft YaHei', serif";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ...
      ctx.fillStyle = Number.isFinite(s.dist[item.id]) ? "#d63b1d" : "#94a3b8";
    ...
    if (hoverId != null) {
      ...
      ctx.fill();
      ctx.strokeStyle = "rgba(214, 59, 29,0.22)";
      ctx.stroke();
      ctx.fillStyle = "#7c2d12";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
